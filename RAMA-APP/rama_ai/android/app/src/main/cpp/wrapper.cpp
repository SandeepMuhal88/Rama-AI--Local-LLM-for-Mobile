...
const char *run_model_path(const char *model_path, const char *prompt) {
  ...
  int n_tokens =
      llama_tokenize(vocab, prompt, n_prompt, nullptr, 0, true, false);
  if (n_tokens < 0)
    n_tokens = -n_tokens;
  if (n_tokens == 0) {
    llama_free(ctx);
  ...
  int rc = llama_tokenize(vocab, prompt, n_prompt, tokens.data(), n_tokens,
                          true, false);
  if (rc < 0) {
    llama_free(ctx);
    llama_model_free(model);
  ...
  if (llama_decode(ctx, batch) != 0) {
    ...
    llama_free(ctx);
    llama_model_free(model);
    return strdup("Error: Failed to process prompt. The model may be "
  ...
  const int max_gen = 512; // keep output short to avoid OOM during generation
  ...
  LOGI("Done. Output length=%zu generation_ok=%d", output.size(),
       (int)generation_ok);

  if (output.empty()) {
    return strdup(
        "(The model produced no output. Try a different model or prompt.)");
  }
